...
# 감지된 원본 언어(source) -> TTS에 쓸 보이스 매핑
# DragonHD 멀티링구얼 보이스는 입력 텍스트 언어에 맞춰 자동으로 발음하므로,
# "번역 대상 언어"가 아니라 "누가 말했는지(번역 방향)"에 따라 목소리를 고정합니다.
VOICE_BY_SOURCE_LANG = {
    "en-US": "en-US-Ava:DragonHDLatestNeural",     # 영어 -> 한국어 번역: 여자 목소리
    "ko-KR": "en-US-Andrew:DragonHDLatestNeural",  # 한국어 -> 영어 번역: 남자 목소리
}
...
def two_way_interpreter():
    ...
    # --- 방향별 TTS synthesizer 미리 준비 (source 언어 기준) ---
    synthesizers = {
        source_lang: build_synthesizer(speech_key, endpoint, voice)
        for source_lang, voice in VOICE_BY_SOURCE_LANG.items()
    }
    ...
    def on_recognized(evt: speechsdk.translation.TranslationRecognitionEventArgs):
        ...
        # 감지된 원본 언어(누가 말했는지)에 따라 고정된 보이스로 읽어줌
        synthesizer = synthesizers[detected_lang]
        print(f"[DEBUG] TTS 시도: source={detected_lang}, text={translated_text!r}")
